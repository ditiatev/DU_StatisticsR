...
ggplot(data = sfo_feb_flights, aes(x = arr_delay/60)) +
  geom_histogram()
```
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median_arr_delays = median(arr_delay), IQR_arr_delays = IQR(arr_delay)) %>%
  arrange(desc(IQR_arr_delays))
```
...
nycflights %>%
  summarise(arr_on_time_dep_delayed = sum(dep_type == "delayed" & arr_type == "on time") / sum(dep_type == "delayed"))
```
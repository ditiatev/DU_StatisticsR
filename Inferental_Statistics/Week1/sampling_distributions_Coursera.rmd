...
# type your code for the Exercise here, and Run Document
sample_means50 %>%
  summarise(x_mean = mean(x_bar), s_med = median(x_bar), 
            SE = sd(x_bar), s_iqr = IQR(x_bar),
            s_min = min(x_bar), s_max = max(x_bar),
            s_q1 = quantile(x_bar, 0.25),
            s_q3 = quantile(x_bar, 0.75))
```
...
sample_means50 <- ames %>%
  ...
  summarise(x_price = mean(price))
```


Plot sampling distribution by 5000 samples from the population of size 50 and computing 5000 sample means.

```{r plot_sample_price_distribution, echo=FALSE}
ggplot(sample_means50, aes(x = x_price)) +
...
sample_means15 <- ames %>%
  ...
  summarise(x_price = mean(price))
ggplot(sample_means15, aes(x = x_price)) +
  geom_histogram(binwidth = 5000)
```
...
sample_means150 <- ames %>%
  ...
  summarise(x_price = mean(price))
ggplot(sample_means150, aes(x = x_price)) +
  geom_histogram(binwidth = 5000)
```
...
sample_means150 %>%
  summarise(sd_150 = sd(x_price), mean_150 = mean(x_price))

sample_means15 %>%
  summarise(sd_15 = sd(x_price), mean_15 = mean(x_price))
```
